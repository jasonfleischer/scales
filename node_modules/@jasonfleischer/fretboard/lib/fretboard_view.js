...
class FretboardView {
	...
	drawHoverNote(note) {
		...
		this.drawNoteWithColor(note, label, this.darkMode ? "#888" : "#eee", canvas);
	}
	...
	addClickEventListeners(onClick) {
		...
		let WIDTH = this.WIDTH;
		var radius = this.radius * (WIDTH/width);
	...
	addHoverEventListeners() {
		...
		view.removeEventListener("mouseover", this.mouseOverFunction(self), true);
		
		this.mouseOverFunction = function(self) {
		    return function curried_func(event) {
		        self.previousMidiValue = undefined;
		    }
		}
		view.addEventListener("mouseover", this.mouseOverFunction(self), true);
		/*const mouseOverFunction = function(event) {
			previousMidiValue = undefined;
		}*/

		view.removeEventListener("mousemove", this.mouseMoveFunction(self), true);
		
		this.mouseMoveFunction = function(self){
			return function curried_func(event) {
		...
		view.addEventListener("mousemove", this.mouseMoveFunction(self), true);
		/*const mouseMoveFunction = function(event){
		    let position =	self.getPosition(view);
		    let x = (event.clientX - position.x) * (WIDTH/width);
		    let y = (event.clientY- position.y) * (WIDTH/width);
		    let clickPoint = new Point(x, y);
		...
		view.removeEventListener("mouseout", this.mouseOutFunction(self), true);
		this.mouseOutFunction = function(self) {
		    return function curried_func(event) {
		        if(self.previousMidiValue !== undefined)
  					self.clearHoverNote(musicKit.all_notes[previousMidiValue]);
		    }
		}
		/*var mouseOutFunction = function(self) {
		    if(previousMidiValue !== undefined)
  				self.clearHoverNote(musicKit.all_notes[previousMidiValue]);
		}*/
		...
		view.addEventListener("mouseout", this.mouseOutFunction(self), true);
	}
