...
class FretboardView {
	...
	onClick(event){
	    ...
	    for (const [key, value] of Object.entries( this.noteValueToNotePositionsDict)) {
	    	let midi_value = key;
	...
	addHoverEventListeners() {
		...
		view.removeEventListener("mouseover", mouseOverFunction);
		view.addEventListener("mouseover", mouseOverFunction);

		view.removeEventListener("mousemove", mouseMoveFunction);
		view.addEventListener("mousemove", mouseMoveFunction);

		view.removeEventListener("mouseout", mouseOutFunction);
		view.addEventListener("mouseout", mouseOutFunction);
	}

	mouseOverFunction(event) {
		this.previousMidiValue = undefined;
	}
	...
	mouseMoveFunction(event){
	    ...
	    let position =	this.getPosition(view);
	    let x = (event.clientX - position.x) * (WIDTH/width);
	    let y = (event.clientY- position.y) * (WIDTH/width);
	    let clickPoint = new Point(x, y);
	    var foundMidiValue = -1;
		...
		if(foundMidiValue != -1){
			...
			if(this.previousMidiValue === undefined) {
	    		previousMidiValue = foundMidiValue;	
	   			this.drawHoverNote(musicKit.all_notes[foundMidiValue]);
	    	}
	    	if(this.previousMidiValue !== foundMidiValue) {
	    		this.clearHoverNote(musicKit.all_notes[this.previousMidiValue]);
	    		previousMidiValue = foundMidiValue;	 		   		
    			this.drawHoverNote(musicKit.all_notes[foundMidiValue]);
	    	}
		} else {
